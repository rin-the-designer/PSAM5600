#!/usr/bin/env python3
"""
Script to extract lyrics with member tags from lyrics_member.html and create lyrics_member.json
"""
...
def extract_member_lyrics(html_file):
    """
    Extract lyrics with member tags from HTML file
    
    Args:
        html_file (str): Path to the HTML file
        
    Returns:
        list: List of songs with member-tagged lyrics
    """
    ...
    member_colors = {
        '#8c00d1': 'Jisoo',  # Purple
        '#4b89dc': 'Jennie', # Blue  
        '#4d89dc': 'Jennie', # Blue (alternative)
        '#fdc3c3': 'Rosé',   # Pink
        '#ffc3c3': 'Rosé',   # Pink (alternative)
        '#ffd400': 'Lisa',   # Yellow
    ...
    try:
        ...
        for album in albums:
            ...
            for song in album['songs']:
                ...
                print(f"    Total lines: {len(song['lyrics'])}")
                
                # Count lines per member
                member_counts = {}
                for line in song['lyrics']:
                    member = line['member']
                    if isinstance(member, list):
                        # For ALL sections, count each member
                        for m in member:
